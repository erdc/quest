...
def test_new_collection():
    ...
    assert len(list(dsl.api.get_collections().keys())) == 4
    ...
    assert len(list(dsl.api.get_collections().keys())) == 5
    assert list(c.values())[0] == {
                        'display_name': 'my test collection', 
                        'tag': 'test tag', 
                        'path': os.path.join(collection_dir, list(c.keys())[0]),
                    }
...
def test_delete_collection():
    ...
    assert len(list(dsl.api.get_collections().keys())) == 3
